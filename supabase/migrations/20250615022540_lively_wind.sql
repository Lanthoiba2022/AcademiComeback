/*
  # Fix database policies and functions - Handle dependencies correctly

  1. Functions
    - Drop triggers first, then functions to avoid dependency errors
    - Recreate functions with proper signatures
    - Add room code generation and user statistics functions
...
BEGIN
  FOR room_record IN SELECT id FROM rooms WHERE code IS NULL OR code = '' LOOP
    UPDATE rooms SET code = generate_room_code() WHERE id = room_record.id;
  END LOOP;
END $$;