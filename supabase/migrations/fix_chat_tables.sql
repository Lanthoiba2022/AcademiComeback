...
-- Drop all existing chat-related objects
...
DROP TRIGGER IF EXISTS update_chat_presence_updated_at ON chat_presence;
...
-- Add foreign key for chat_settings
ALTER TABLE chat_settings 
ADD CONSTRAINT fk_chat_settings_room_id 
FOREIGN KEY (room_id) REFERENCES rooms(id) ON DELETE CASCADE;
...
-- Add constraints for chat_presence
ALTER TABLE chat_presence 
ADD CONSTRAINT chat_presence_status_check 
CHECK (status IN ('available', 'away', 'busy', 'offline'));

ALTER TABLE chat_presence 
ADD CONSTRAINT fk_chat_presence_room_id 
FOREIGN KEY (room_id) REFERENCES rooms(id) ON DELETE CASCADE;

ALTER TABLE chat_presence 
ADD CONSTRAINT fk_chat_presence_user_id 
FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
-- Add foreign keys for chat_typing_indicators
